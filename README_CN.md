...
- **快速插入**：支持多种符号，用于提升代码可读性。
  - `Ctrl/Cmd + -` (短横线) | `Ctrl/Cmd + =` (等号) | `Ctrl/Cmd + Shift + 8` (星号) | `Ctrl/Cmd + S` (自定义)。
- **智能包裹模式**：
  - **空行插入**：生成完整宽度的分割线（长度可在设置中调整）。
...
5. **注释样式** (`stata-outline.commentStyle`)

   - `// `（默认）：双斜杠行注释。
   - `* `：Stata 单行注释。
   - `/* ... */`：块状注释。
6. **分割线长度** (`stata-outline.separatorLength`)
...
| 版本        | 更新内容                                                 | 发布日期   |
| ----------- | -------------------------------------------------------- | ---------- |
| 0.2.0       | macOS 上无需额外依赖即可运行代码；新增分割线命令与快捷键 | 2026-01-25 |
